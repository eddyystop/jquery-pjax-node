...
PJAX.qs = { // source shared with PJAX.qs in nodejs server examples
  ...
  parseQs: function parseQuery (query, isHash) {
    ...
    function createElement(params, key, value) {
      ...
      if (key.indexOf('.') !== -1) {
        ...
        if (index == '') {
          params[key].push(PJAX.qs.coerceStr(value));
        } else {
          params[key][parseInt(index, 10)] = PJAX.qs.coerceStr(value);
        }
      } else {
        // just normal key

        params[key] = PJAX.qs.coerceStr(value);
      }
    }
  },

  coerceStr: function (str) {
...
PJAX.link = {
  ...
  pjaxLinkRoutes: function pjaxLinkRoutes (containerId, config) {
    ...
    $('#' + containerId).find('a[pjax-route]').click(function (event) {
      ...
      console.log('<> PJAX.link url=' + urlAndContainer[0] +
        '; container=' + urlAndContainer[1]);
      $.pjax({
  ...
  getRouteUrlAndContainer: function (routeName, config) {
    ...
    return [path + qs, route.container || ''];
  },
  ...
  getObjProp: function getObjProp (obj /* args */) {
    var value = obj;
  ...
  _submitPjaxForm: function _submitPjaxForm (containerId, getIsFormValid,
    ...
    $form.submit(function (event) { // event auto removed when HTML deleted
      ...
      if (isFormValid) {
        ...
        console.log('<> PJAX.form.submit url=' + $form.attr('action') +
